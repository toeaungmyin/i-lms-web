<div class="flex flex-col justify-center items-start gap-4 mt-4">
    <div id="quizzes-container" class="w-full">
        @foreach ($course->questions as $key => $question)
            <form class="w-full" id="quizz-form-{{ $question->id }}" action="{{ route('dashboard.quizzes.update',$question->id) }}" method="POST">
                @csrf
                @method('PUT')
                            ...
                            <div class="flex flex-wrap items-center divide-gray-200 sm:divide-x sm:rtl:divide-x-reverse dark:divide-gray-600">
                                <div class="flex items-center space-x-1 rtl:space-x-reverse sm:ps-2">
                                    <div class="w-full flex justify-center items-center gap-4">
                                        ...
                                        <input type="text" name="answer" id="answer" class="bg-white border border-gray-300 text-green-500 text-sm font-bold font-mono rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2" required value="{{ $question->answer }}" />
                                    </div>
                                </div>
                            </div>
                            <x-primary-button id="quizz-form-submit-btn-{{ $question->id }}">
